...
$syncOverridesScript = $paths.SyncOverridesScript
...
if (Test-Path -LiteralPath $syncOverridesScript) {
...
Write-Host "   PowerShell -ExecutionPolicy Bypass -File .\scripts\credentials\save_longzhi_credential.ps1 -Username `"YOUR_USER`" -Password `"YOUR_PASS`""
Write-Host "   PowerShell -ExecutionPolicy Bypass -File .\scripts\credentials\save_siliconflow_credential.ps1 -ApiKey `"YOUR_SILICONFLOW_KEY`""
